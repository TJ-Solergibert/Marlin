...
#SBATCH --environment=/capstor/scratch/cscs/asolergi/Marlin/container/datatrove.toml # WARN(tj.solergibert) Modify path to your own file
#SBATCH --no-requeue
...
start_s=`date`
start=`date +%s`
numactl --membind=0-3 python3 $MARLIN_DIR/scripts/tokenization/preprocess_megatron.py --tokenizer-name-or-path $tokenizer --output-folder $output_folder --logging-dir $logging_dir --n-tasks $number_of_tasks --dataset $input_folder --paths-file $paths_file
end=`date +%s`
end_s=`date`
...
# Stats
wc=$((end-start))
dataset_total_size=$(python3 $MARLIN_DIR/scripts/tokenization/compute_dump_size.py $paths_file)
